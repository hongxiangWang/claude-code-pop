...
- 降级 Recovery CLI 模式（`CLAUDE_CODE_FORCE_RECOVERY_CLI=1 ./bin/pop-claude`）
...
```bash
./bin/pop-claude                          # 交互 TUI 模式
./bin/pop-claude -p "your prompt here"    # 无头模式
./bin/pop-claude --help                   # 查看所有选项
```
...
| [全局使用](docs/guide/global-usage.md) | 在任意目录启动 pop-claude |
